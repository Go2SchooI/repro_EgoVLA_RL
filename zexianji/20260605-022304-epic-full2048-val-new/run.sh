...
VAL_CONFIG=$EXP_DIR/config.yaml
...
if [ ! -x "$PYTHON_BIN" ]; then echo "missing python: $PYTHON_BIN" >&2; exit 2; fi
if [ ! -d "$REPO" ]; then echo "missing repo: $REPO" >&2; exit 2; fi
for ckpt in "${CKPTS[@]}"; do
  if [ ! -f "$ckpt/weights.pt" ]; then echo "missing checkpoint weights: $ckpt/weights.pt" >&2; exit 2; fi
done
(cd "$REPO" && git rev-parse HEAD > "$EXP_DIR/env/git_head.txt" 2>/dev/null || true)
(cd "$REPO" && git status --short > "$EXP_DIR/env/git_status.txt" 2>/dev/null || true)
nvidia-smi > "$EXP_DIR/env/nvidia-smi.txt" 2>&1 || true
...
if [ "$NUM_SHARDS" -lt 1 ]; then echo "NUM_SHARDS must be >= 1" >&2; exit 2; fi
if [ "$NUM_SHARDS" -gt "${#GPUS[@]}" ]; then echo "NUM_SHARDS=$NUM_SHARDS exceeds GPU count/list ${#GPUS[@]}" >&2; exit 2; fi
progress_snapshot() { "$EXP_DIR/show_progress.sh" "$1" || true; }
monitor_progress() {
  local label="$1"
  while true; do
    local alive=0
    ...
    progress_snapshot "$label" | tee -a "$EXP_DIR/logs/progress.log"
    [ "$alive" -eq 0 ] && break
    sleep "$PROGRESS_INTERVAL"
  done
}
echo "start_time=$(date -Is)" | tee "$EXP_DIR/logs/run.log"
echo "mode=all_windows_from_fixed_2048_episodes dataset_size=$DATASET_SIZE max_windows=0 num_shards=$NUM_SHARDS gpus=${GPUS[*]} batch_size=$BATCH_SIZE num_workers=$NUM_WORKERS" | tee -a "$EXP_DIR/logs/run.log"
for idx in "${!LABELS[@]}"; do
  ...
  PIDS=()
  SHARD_DIRS=()
  mkdir -p "$EXP_DIR/artifacts/shards/$RUN_LABEL"
  for ((shard=0; shard<NUM_SHARDS; shard++)); do
    gpu=${GPUS[$shard]}
    run_name=${RUN_LABEL}-full2048-shard${shard}of${NUM_SHARDS}
    ...
    (
      cd "$REPO"
      CUDA_VISIBLE_DEVICES=$gpu "$PYTHON_BIN" validation/run_vitra.py \
        --config "$VAL_CONFIG" \
        --checkpoint "$CKPT" \
  ...
  progress_snapshot "$RUN_LABEL" | tee -a "$EXP_DIR/logs/progress.log"
  if [ "$rc" -ne 0 ]; then
  ...
  (
    cd "$REPO"
    "$PYTHON_BIN" validation/scripts/merge_vitra_shards.py "${SHARD_DIRS[@]}" --output-dir "$MERGED" --top-k 20 --require-full-coverage
...
lines=['# Results','', '## Status','success','', '## Command','```bash','./run.sh','```','', '## Timing', f'- End: {time.strftime("%Y-%m-%d %H:%M:%S %z")}', '', '## Metrics', '- mode: fixed 2048 EPIC seen-val episodes, all expanded windows']
for label in labels:
