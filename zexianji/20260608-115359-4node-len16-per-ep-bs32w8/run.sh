...
# Lightweight sandbox wrapper for SG DLC 4-node LEN16 per-episode-file training + EPIC-only validation.
...
# Requested scale: 4 nodes x 8 GPUs, per-GPU bs=32, workers=8.
...
export RUN_NAME="${RUN_NAME:-epic100-ego4d-len16-per-ep-bs32x32-w8-epoch${MAX_EPOCHS}-$(date -u +%Y%m%dT%H%M%SZ)}"
export WANDB_RUN_VARIANT="${WANDB_RUN_VARIANT:-epic100-ego4d-len16-per-ep-epoch2-bs32x32-w8}"
export WANDB_TAGS="${WANDB_TAGS:-stage=scaling-law,backbone=paligemma2,dataset=epic100-plus-ego4d-len16,variant=ego4d-len16-per-ep-bs32w8,scale=len16,bs=32,gpus=32,workers=8,read=full}"
export WANDB_NOTES="${WANDB_NOTES:-[epic100-ego4d-len16-per-ep-bs32w8] EPIC100 fixed + ego4d screened-len16; data root=<DATA_ROOT>; per_gpu_bs=32; total_batch=1024; workers=8; max_epochs=2; full parquet read; validation=current EPIC seen-val max_windows=3000.}"

ENV_FILE="${EXP_DIR}/env/launch_env_runtime.txt"
{
  echo "created=2026-06-08 11:53:59 Asia/Shanghai"
